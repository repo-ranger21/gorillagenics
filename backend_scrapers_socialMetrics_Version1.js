...
class SocialMetricsScraper {
  ...
  async scrapePlayerPosts(playerName) {
    ...
    try {
      // Mock Instagram/Twitter scraping - replace with actual implementation
  ...
  async extractBiometrics(post, playerName) {
    ...
    for (const pattern of this.sleepPatterns) {
      ...
      if (match) {
        ...
        metrics.sleepScore = normalizeMetric(sleepHours * 10, 'sleep'); // 8hrs = 80pts
        break;
      }
    }
    ...
    // OCR processing for images with WHOOP/Oura screenshots
