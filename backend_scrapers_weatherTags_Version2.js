...
class WeatherTagsScraper {
  ...
  generateWeatherTags(temp, humidity, conditions) {
    const tags = [];
    if (temp > 85) tags.push('high_heat');
    if (temp < 32) tags.push('freezing');
    if (humidity > 80) tags.push('high_humidity');
    if (conditions.includes('rain')) tags.push('wet_conditions');
    if (conditions.includes('snow')) tags.push('snow');
    if (conditions.includes('wind')) tags.push('windy');
    return tags;
  }
  ...
  generateWeatherCommentary(temp, conditions) {
    if (temp > 90) return "🦍 Scorching conditions ahead. Hydration stations on standby.";
    if (temp < 25) return "🦍 Frozen tundra detected. Prepare for ice-cold domination.";
    if (conditions.includes('rain')) return "🦍 Wet and wild conditions. Fumble alert activated.";
